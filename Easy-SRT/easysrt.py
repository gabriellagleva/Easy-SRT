...
# Windows: Add NVIDIA DLL directories to PATH so CTranslate2 can find cublas, cudnn, etc.
# Pip-installed NVIDIA packages place DLLs in site-packages/nvidia/*/bin/
# which isn't on the default search path. This is a Windows-only issue.
if sys.platform == "win32":
    _nvidia_dir = Path(sys.prefix) / "Lib" / "site-packages" / "nvidia"
    if _nvidia_dir.exists():
        _nvidia_bins = [str(p) for p in _nvidia_dir.glob("*/bin") if p.is_dir()]
...
def main():
    ...
    print("Initalizing and / or downloading Whisper Model. This can take several minutes.")
